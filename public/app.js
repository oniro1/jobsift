...
async function toggleSave(jobId) {
  ...
  if (saved) { state.savedJobs.delete(jobId); }
  else {
    state.savedJobs.add(jobId);
    try { await fetch('/api/user/save-job', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}` }, body: JSON.stringify({ jobId }) }); } catch {}
  }
...
async function login() {
  ...
  try {
    ...
    saveAuth(data); closeModalDirect();
...
async function register() {
  ...
  if (password.length < 8) {
    document.getElementById('reg-error').textContent = 'Password must be at least 8 characters.';
    return;
  }
...
function logout() {
  ...
  updateNav(); showToast('Signed out');
}
